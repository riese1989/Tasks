...
public class OperationsEmployee {
    ...
    public boolean stat() {
        ...
        System.out.println("\nОбщий счёт");
        for (Employee employee : repoEmployees.get()) {
            long countTasks = Access.getRepoTasks().get().stream().filter(t -> t.getAssigned().getFamily().equals(employee.getFamily())).count();
    ...
    public Employee tasksOfAuthor(String author) {
        for (Employee employee : repoEmployees.get()) {
            Long count = Access.getRepoTasks().get().stream().filter(t -> t.getAssigned() == employee && t.getAuthor().equals(author)).count();
